...
function run-conversation-history-cleanup {
    ...
    Process
    {
        ...
        $str_start_date = ([datetime]::parseexact($start_date, 'dd.MM.yy', $null).ToString('MM/dd/yyyy')).Replace('.','/')
        $date_range_filter = "[CreationTime] > '{0:$str_start_date}'" #mm/dd/yyyy 
        ...
        Write-Host "$total_items_count number of items found in Conversation History folder of $email mailbox. " -ForegroundColor Green
        Write-Host "$filtered_items_count number of items will be processed." -ForegroundColor Green
        ...
        foreach ($item in $conversation_history.Items.Restrict($date_range_filter)) {
            ...
            $item_date = $item.CreationTime
            $item_entry_id = $item.EntryID
            $size = $item.size
 
            $object = [PSCustomObject]@{
                conversation_id = $conversation_id
                creation_time = $item_date
        ...
        foreach ($item in $conversation_history_items) {
     
            Write-Progress -Activity “Analyzing items” -status “Processing item $i of $filtered_items_count” ` -percentComplete ($i / $filtered_items_count *100)
            $duplicates = ""
            $duplicates = $conversation_history_items.Where({$_.conversation_id -eq $item.conversation_id -and $_.creation_time -ne $item.creation_time})
            $conversation_history_items = $conversation_history_items.Where({$_.entry_id -NotIn $duplicates.entry_id})
...
run-conversation-history-cleanup -email (Read-Host -Prompt "Enter Outlook mailbox name") -action (Read-Host -Prompt "Enter desired action (analyze/delete)") -start_date (Read-Host -Prompt "Enter start date for filtering (use dd.MM.yy format)")
